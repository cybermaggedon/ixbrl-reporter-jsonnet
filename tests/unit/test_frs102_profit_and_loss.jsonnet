// Phase 2: Unit test for lib/frs102/profit-and-loss.libsonnet
local test = import "jsonnetunit/test.libsonnet";
local profit_and_loss = import "lib/frs102/profit-and-loss.libsonnet";

test.suite({
    testProfitAndLossIsObject: {
        actual: std.type(profit_and_loss),
        expect: "object"
    },

    testHasElementMethod: {
        actual: std.objectHasAll(profit_and_loss, 'element'),
        expect: true
    },

    testElementMethodIsFunction: {
        actual: std.type(profit_and_loss.element),
        expect: "function"
    },
    ...
    testElementReturnsValidStructure: {
        local mock_context = {},
        local result = profit_and_loss.element(mock_context),
    ...
    testWithIdBuilder: {
        local mock_context = {},
        local result = profit_and_loss.element(mock_context).with_id("test-id"),
