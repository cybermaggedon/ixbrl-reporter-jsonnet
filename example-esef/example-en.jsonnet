...
// ESEF (European Single Electronic Format) English Language Example
...
// Import the ESEF English language library
...
local accts = {
    metadata: import "example-esef/metadata.jsonnet",
    accounts:: l.from_element_def(elts, self).with_metadata(self.metadata),
    resource(x):: {
	"logo": import "example-esef/logo.jsonnet",
	"notes": notes
    }[x]
};
